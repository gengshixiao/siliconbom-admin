...
function renderTable() {
  ...
  pageData.forEach(user => {
    ...
    row.innerHTML = `
      ...
      <td><button class="btn-action" onclick="showSessions('${user.id}', '${user.username}')">查看会话</button></td>
    `;
...
function showSessions(userId, username) {
  const modal = new Modal('sessionModal');
  const sessionList = document.getElementById('sessionList');
  
  // 模拟会话数据
  const sessions = [
    { time: '2025-01-15 14:20', content: '如何查询BOM中的某个器件的规格参数？' },
    { time: '2025-01-15 13:45', content: '帮我分析一下这个电路的设计是否合理' },
    { time: '2025-01-14 16:30', content: '知识库中有关于EMI测试的相关文档吗？' },
    { time: '2025-01-14 10:15', content: '对比一下这两个器件的性能差异' },
    { time: '2025-01-13 15:20', content: '帮我生成一份BOM清单' }
  ];
